        ...
        <div class="scrollbar-sidebar mt-4">
          <div class="app-sidebar__inner">
            <ul class="vertical-nav-menu">
              <li>
                <router-link to="/tfmbddash" class="mm-active">
                    BD
                </router-link>
              </li><br>
              <li>
                <router-link to="/tfmprojdash" class="">
                    Study
                </router-link>
              </li>
            </ul>
      ...
      <div class="app-main__outer">
        <div class="app-main__inner">
          <div class="row bg-light rounded" v-for="detail in details" :key="detail.id">
            ...
            <div class="col-3 mb-4 mt-4 text-right">
              <label class="text-primary font-weight-bold">Status: </label>
            </div>
            <div class="col-3 mb-4 mt-4">
              <select class="form-control" v-model="tfm_status">
            ...
            <div class="col-12 mb-2">
              <label class="text-primary font-weight-bold">Comment from TFM</label><br>
              <textarea class="form-control" rows="4" v-model="tfm_comments"></textarea>
            </div>
            <div class="col-12 mb-2">
              <label class="text-primary font-weight-bold">Previous Comments from BD</label><br>
              <p>{{detail.bd_comments}}</p>
            </div>
            <div class="col-12 mb-2">
              <label class="text-primary font-weight-bold">Previous Comments from TFM</label><br>
              <p>{{detail.tfm_comments}}</p>
            </div>
            <div class="mb-3 mt-2 text-right">
            <button type="button" class="btn btn-success" @click="save">Send</button>
            </div>
          </div>      
        </div>    
      </div>
...
export default {
  ...
  methods: {
    ...
    save: function() {
      let uri = "/api/tfmbdupdate/" + this.$route.params.id;
      axios.post(uri, {
        tfm_status: this.tfm_status,
        tfm_comments: this.tfm_comments
      }).then((res) => {
          alert("Successfully Saved!");
          window.location.href='/tfmbddash';
      }).catch(()=> {
